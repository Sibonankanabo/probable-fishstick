...
PREDICTION_INTERVAL = 7  # Make predictions at every 7 intervals
...
class TraderStrategy(Strategy):
    ...
    window_size =  10   # Number of bars to use in each prediction window
    ...
    def next(self):
        ...
        if TraderStrategy.X_new_data is not None and len(self.data) >= self.window_size and self.counter % PREDICTION_INTERVAL == 0:
            ...
            # Reshape predicted_diff if needed
            predicted_diff = np.reshape(predicted_diff, (1, -1))
...
def run_backtest(login_id, password, server, symbol):
    ...
    if historical_data is not None and isinstance(historical_data, pd.DataFrame):
        ...
        X_new_data =  X_new_data.drop(columns=[0, 6, 7, 8, 9],errors='ignore')
        
        X_new_data.columns = ['Open', 'High', 'Low', 'Close', 'Volume']
        
        # Run backtest using fetched data
        bt = Backtest(X_new_data, TraderStrategy, cash=10, commission=0.05)
        output = bt.run()
